...
@dataclass(frozen=True)
class CandidateV2Pool:
    from_token: Token
    to_token: Token
    exists: bool
    from_reserve: Wei = Wei(0)
    to_reserve: Wei = Wei(0)
    decimal_adjusted_to_per_from_exchange_rate: float = 0.0


@dataclass(frozen=True)
class CandidateV3Pool:
    from_token: Token
    to_token: Token
    exists: bool
    fee: int = 0
    from_balance: Wei = Wei(0)
    to_balance: Wei = Wei(0)
    decimal_adjusted_to_per_from_exchange_rate: float = 0.0
...
V3PathList = tuple[int | ChecksumAddress, ...]
...
class V2PoolPath(PoolPath[V2OrderedPool, V2PathList]):
    ...
    def __init__(
        self, pools: Sequence[V2OrderedPool], smart_rate_limiter: SmartRateLimiter | None = None
    ) -> None:
        self.pools = pools
...
class V3PoolPath(PoolPath[V3OrderedPool, V3PathList]):
    ...
    def __init__(
        self, pools: Sequence[V3OrderedPool], smart_rate_limiter: SmartRateLimiter | None = None
    ) -> None:
        self.pools = pools
        self.path = self._build_path()
        self.smart_rate_limiter = smart_rate_limiter
    ...
    def _build_path(self) -> V3PathList:
        path: list[int | ChecksumAddress] = [self.pools[0].token_in.address]
        for pool in self.pools:
    ...
    async def get_amount_out(self, amount_in: Wei) -> Wei:
        ...
        # Auto-detect based on return type
        if isinstance(quote, (list, tuple)):
            # QuoterV2 returns a tuple/list (amountOut, sqrtPriceX96AfterList, initializedTicksCrossedList, gasEstimate)
            return to_wei(quote[0])
        else:
            # QuoterV1 returns just amountOut as a single value
            return to_wei(quote)
...
class WeightedPath:
    ...
    def to_dict(self) -> dict[str, str | PathList | int]:
...
class MixedWeightedPath:
    ...
    async def compute_path_values(self, amount: Wei) -> None:
        computing_coros: list[Coroutine[Any, Any, Wei]] = [
            w_p.pool_path.get_amount_out(Wei(amount * w_p.weight // 100)) for w_p in self.weighted_paths
        ]
        try:
            ...
            raise ValueError(
                f"Timeout while computing path values for {len(self.weighted_paths)} path(s): {e}"
            ) from e
        except (ValueError, Web3Exception) as e:
            raise ValueError(
                f"Error computing path values for {len(self.weighted_paths)} path(s): {e}"
            ) from e
