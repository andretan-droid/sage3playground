...
def demote():
    """Drop privileges to uid/gid 1000.

    Intended as a `preexec_fn` so model-issued commands cannot read the MCP
    server directory or interfere with the root-owned anvil process.
    """
    os.setgid(1000)
    os.setuid(1000)
...
async def run(
    cmd: str,
    timeout: float | None = 120.0,  # noqa: ASYNC109
